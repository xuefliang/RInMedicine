...
例15 某实验器材损坏率不超过5%. 现要估计某批产品中的损坏率,要求估计与真值间不超过1%, 且置信度为0.90, 那么要抽取多少样本验收可满足上述要求?
...
例16 描述性调查研究时，某病的患病率估计为30%,95%置信区间不超过患病率的5%，计算所需的样本容量。
```{r}
size.bin(0.05,0.3)
```
###两样本估计正态总体均值时样本容量的确定
常通过假设检验的公式反推而得到。
####两样本总体均数的检验或配对样本均数的检验
其中一个样本的置信度为$1-\alpha$,另一个样本的置信度为$1-\beta$最大容许误差为$d$,样本容量为$n=\left ( \frac{(z_{1-\alpha /2}+z_{1-\beta/2})s}{d} \right )^{2}$。
 
例17 用某药治疗高胆固醇血症。已知：血清高胆固醇平均降低0.52mmol/L在专业上认为有意义。若差值的标准差为1.28mmol/L，$\alpha=0.05$,$\beta=0.1$，问需要多大的样本容量？
```{r}
...
####两样本均数检验
其中一个样本的置信度为$1-\alpha$,另一个样本的置信度为$1-\beta$最大容许误差为$d$,样本容量为$n=\left ( \frac{(z_{1-\alpha /2}+z_{1-\beta/2})s}{d} \right )^{2}((\frac{n_{1}}{N})^{-1}+(\frac{n_{2}}{N})^{-1})$,$\frac{n_{1}}{N}$和$\frac{n_{3}}{N}$为样本比例。
例18 比较A、B两药降低收缩压效果。设两药降低效果的差值为5mmHg及以上为专业有意义？已知：12mmHg,$\alpha=0.05$取双侧检验,$\beta=0.1$取单侧检验，问每组例数相等，则每组需要多少病例？若B组样本量是A组的2倍，则每组又各需要多少病例？
```{r}
size.norm4 <- function(d,s,alpha,beta,p1,p2){
  ((qnorm(1-alpha/2)+qnorm(1-beta))*s/d)^2*((p1)^-1+(p2)^-1)
}
