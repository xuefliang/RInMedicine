...
K期Wang and Tsiatis 检验的样本量计算，首先根据没有期中分析的设计计算最大样本量，然后乘以$R_{wt}(K,\alpha ,\beta,\Delta)$。最大样本量计算公式如下$$n_{fixed}=\frac{(z_{1-\alpha /2}+z_{1-\beta })^{2}(\sigma_{1}^{2}+\sigma_{2}^{2})}{(\mu _{1}-\mu _{2})^{2}}$$,$R_{wt}$可查询下表：
...
K期Inner Wedge 检验的样本量计算，首先根据没有期中分析的设计计算最大样本量，然后乘以$R_{w}(K,\alpha ,\beta,\Delta )$。最大样本量计算公式如下$$n_{fixed}=\frac{(z_{1-\alpha /2}+z_{1-\beta })^{2}(\sigma_{1}^{2}+\sigma_{2}^{2})}{(\mu _{1}-\mu _{2})^{2}}$$,$R_{w}$可查询下表：
