...
###时间事件数据（生存分析）
以时间事件为试验结果的成组序贯设计，为简单起见，仅Cox比例风险模型为例。固定样本量计算公式,$$I_{fixed}=\frac{(z_{1-\alpha /2}+z_{1-\beta })^{2}}{\theta ^{2}}$$,最大样本计算公式$$I_{max}=I_{fixed}\times R_{B}(K,\alpha ,\beta )$$,样本量为$$n_{d}=\frac{I_{max}}{I_{k}}$$。
...
每期各需要`r round(Cox.Test(0.05, 0.2, 0.405, 5))`例。
