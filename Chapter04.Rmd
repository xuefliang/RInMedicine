...
###样本量再估计
在某些成组序贯试验的期中分析时，需要根据累计的数据对样本量进行再估计，应当注意盲目的进行再估计有可能造成偏移。Shih等提出了以率为观察结果的随机双盲样本量再估计方法,估计公式如下:$$n=\frac{(z_{1-\alpha /2}+z_{1-\beta })^{2}(p _{1}(1-p _{1})+p _{2}(1-p _{2}))}{\Delta^{2}}$$。
