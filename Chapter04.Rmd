...
例 研究新型抗血小板药预防脑梗死再发的作用，和阿司匹林进行对照，以相对危险度OR=2.25为主要评价指标。预实验显示，新药能够预防20%的脑梗死再发，阿司匹林能够预防10%的脑梗死再发。在$\alpha=0.05$,$\beta=0.1$，1：1双侧显著性检验，需要多少病例？

```{r}
RelativeRisk.Equality(0.05,0.1,2.25,1,0.2,0.1)
```
试验组和对照组均需要277例。
...
\end{pmatrix}$
例 研究新型抗血小板药预防脑梗死再发的作用，和阿司匹林进行对照，以相对危险度OR=2.25为主要评价指标。预实验显示，新药能够预防20%的脑梗死再发，阿司匹林能够预防10%的脑梗死再发。在$\alpha=0.05$,$\beta=0.1$，1：1双侧非劣效检验，界值为10%，需要多少病例？

```{r}
RelativeRisk.NIS(0.05,0.1,2.25,1,0.2,0.1,-0.1)
```
试验组和对照组均需要179例。
...
例 研究新型抗血小板药预防脑梗死再发的作用，和阿司匹林进行对照，以相对危险度OR=2.25为主要评价指标。预实验显示，新药能够预防20%的脑梗死再发，阿司匹林能够预防10%的脑梗死再发。在$\alpha=0.05$,$\beta=0.1$，1：1双侧等效性检验，界值为10%，需要多少病例？

```{r}
RelativeRisk.Equivalence(0.05,0.1,2.25,1,0.2,0.1,0.1)
```
试验组和对照组均需要372例。