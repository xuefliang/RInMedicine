...
###非分层分析法
$$\tilde{n}=\frac{(\tilde{\sigma_{0}}z_{1-\alpha/2}+\tilde{\sigma_{1}}z_{1-\beta})^{2}}{(p_{1}-p_{2})^{2}}$$,$\delta =\sum _{j=1}^{J}a_{j}b_{j1}b_{j2}(p_{j1}-p_{j2})$,$\sigma _{1}^{2}=\sum_{j=1}^{J}a_{j}b_{j1}b_{j2}(b_{j2}p_{j1}q_{j1}+b_{j1}p_{j2}q_{j2})$,$\sigma _{0}^{2}=\sum_{j=1}^{J}a_{j}b_{j1}b_{j2}(b_{j1}p_{j1}+b_{j2}p_{j2})(b_{j1}q_{j1}+b_{j2}q_{j2})$ J为层数，a为每层的样本数占总样本数的比例$a_{j}=n_{j}/n$，$b_{jk}=n_{jk}/n_{j}$ k为1,2代表采取的处理方式，假设以1组为对照$b_{j1}+b_{j2}=2$，p1为j层k处理的概率。
