...
ISV.Equality <- function (alpha, beta, sigma1, sigma2, m) 
{
  ...
  n =0
  for (i in 1:1000) {
    ratio.f = qf(p = (1 - beta), i * (m - 1), i * (m - 1), 
                 lower.tail = FALSE)/qf(alpha/2, i * (m - 1), i * 
                                          (m - 1), lower.tail = FALSE)
...
ISV.NIS <- function (alpha, beta, sigma1, sigma2, m, margin) 
{
  ...
  for (i in 1:1000) {
    ratio.f = qf(p = (1 - beta), i * (m - 1), i * (m - 1), 
                 lower.tail = FALSE)/qf(alpha, i * (m - 1), i * (m - 
                                                                   1), lower.tail = FALSE)
    if(round(ratio,digits = 2)==round(ratio.f,digits = 2)){
      n=i
    }
...
###简单随机效应模型
Quan和Shih $\sigma _{i}^{*2}=\frac{1}{2m}\hat{CV_{i}^{2}}+\hat{CV_{i}^{4}}$
