...
固定样本设计方案,是传统的随机对照临床试验设计，这种设计要求试验开始前确定样本量, 并且只有当所有的受试者均完成入组之后才能进行数据的统计分析。其缺点是不能在试验过程中进行统计分析,进行组间比较,因此不能早期发现可能出现的两种药物之间的差异。基于期中分析的、具有不同试验中止法则的成组序贯设计方案在期中分析、监测临床试验的有效性和安全性的基础上,允许提前中止临床试验,但试验方案一旦确定在试验期间不允许修改。
