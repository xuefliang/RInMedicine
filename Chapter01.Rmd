...
###  均值(Mean)
一阶原点矩($A_{1}=\frac{1}{n}\sum_{i=1}^{n}X_{i}$),二阶原点矩($A_{2}=\frac{1}{n}\sum_{i=1}^{n}X_{i}^{2}$),一阶中心矩($B_{1}=\frac{1}{n}\sum_{i=1}^{n}(X_{i}-\bar{X})$),二阶中心矩($B_{2}=\frac{1}{n}\sum_{i=1}^{n}(X_{i}-\bar{X})^{2}$)。一阶原点矩又称均数是一组数据的平均值,均数（记为$\bar{x}$）定义为
