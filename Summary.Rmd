...
```{r eval=FALSE}
sudo apt-get install build-essential git-core gfortran libxml2-dev 
libcurl4-openssl-dev libcurl4-gnutls-dev libfreetype6-dev 
libbz2-dev liblapack-dev libpcre++-dev liblzma-dev r-cran-rcpp 
r-cran-rjava openjdk-7-* curl libgmp3-dev libmysql++-dev 
libmpfr-dev libgdal1-dev libproj-dev libglu1-mesa-dev r-cran-boot 
r-cran-class r-cran-cluster 
...
在CRAN上已经下线包，采用离线安装
```{r eval=FALSE}
install.packages("~/Downloads/epicalc_2.15.1.0.tar.gz", 
                 repos = NULL, type = "source")
install.packages("~/Downloads/lmPerm_1.1-2.tar.gz", 
                 repos = NULL, type = "source")
```
...
## TexLive2015安装
```{r eval=FALSE}
sudo mount -t iso9660 -o ro,loop,noauto texlive2015.iso /mnt 
cd /mnt 
sudo ./install-tl 
sudo umount -l /mnt

#安装完成后需要设置环境变量，即在/etc/profile下加入
vi /etc/profile
...
描述样本分散性，数据越分散，其极差越大。
$$R=max(x)-min(x)$$
例 对第一个例子求变异系数、样本校正平方和、样本未校正平方和、极差和均值的标准误。
```{r}
...
cdfcomp(list(fitW, fitg, fitln), 
        legendtext=c("Weibull", "gamma", "lognormal"))
denscomp(list(fitW, fitg, fitln), 
         legendtext=c("Weibull", "gamma", "lognormal"))
qqcomp(list(fitW, fitg, fitln), 
       legendtext=c("Weibull", "gamma", "lognormal"))
ppcomp(list(fitW, fitg, fitln), 
       legendtext=c("Weibull", "gamma", "lognormal"))
gofstat(list(fitW, fitg, fitln), 
...
例 设某药厂一天中发生着火现象的次数X服从参数为$\lambda$的Poisson分布，$\lambda$未知，有以下样本值，试用矩法估计参数$\lambda$。
...
sigma<-var(x) #这里的var并不是样本方差的计算函数，
#而是修正的样本方差，其实也就是x的总体方差
mu
sigma
```
...
例 对MASS包中的geyser数据，该数据采集自美国黄石公园内的一个名叫Old Faithful 的喷泉。“waiting”就是喷泉两次喷发的间隔时间，“duration”当然就是指每次喷发的持续时间。在这里，我们只用到“waiting”数据
```{r}
...
LL<-function(params,data) 
  #定义log-likelihood函数,参数"params"是一个向量，
  #依次包含了五个参数：p,mu1,sigma1,#mu2,sigma2.#参数"data"，是观测数据。
{
...
ll<-sum(log(f))  
#混合密度函数,log-likelihood函数
return(-ll) #nlminb()函数是最小化一个函数的值，
#但我们是要最大化log-likeilhood函数，所以需要在“ll”前加个“-”号。
}
#参数估计
...
#将估计的参数函数代入原密度函数。
hist(geyser$waiting,probability=T,col=0,ylab="Density", 
     #作出数据的直方图
...
mix.model <- normalmixEM(sim.x, lambda = c(0.3, 0.3, 0.4), 
                         mu = c(-20, 0, 20), sigma = c(1, 1, 1), k = 3)
summary(mix.model)
...
例 某单位随机抽样的15位员工的身高分别为: 159 158 164 169 161 161 160 157 158 163 161 154 166 168 159,  假设身高服从方差为4的正态分布, 要求估计该单位员工身高均值的置信区间，置信水平为95%。
...
z.test<-function(x,sigma,conf.level=0.95,u0=0,alt="two.sided"){
  ...
  result$interval<-c(mean-sigma*qnorm(1-a/2,0,1)/sqrt(n),
                     mean+sigma*qnorm(1-a/2,0,1)/sqrt(n))
  result
}
...
例 假设不知道上例中总体的方差，要求估计该单位员工身高均值的置信区间，置信水平为95%。
```{r}
t.test(x)
```
...
例 假设不知道例题5中总体的方差，要求估计该单位员工身高方差的置信区间，置信水平为95%。
```{r}
chisq.var.test<-function(x,conf.level=0.95,alt="two.sided",sigma0=1)   
  #默认95%的置信区间 双侧检验
{
  result<-list()
  n <- length(x)
  a=1-conf.level
  v<-var(x)
  result$interval<-c((n-1)*v/qchisq(1-a/2,n-1,lower.tail=T),
                     (n-1)*v/qchisq(a/2,n-1,lower.tail=T))
  chi2<-(n-1)*v/sigma0
  result$chi2<-chi2
  p<-pchisq(chi2,n-1)
  if(alt=="two.sided")
...
例 为比较两种药品的降糖效果, 选择20名条件相似的受试者, 分别服用甲、乙两种药品后，测得甲组的血糖为6.48 4.00 5.54 6.89 5.14 4.60 3.67 4.32 4.80 7.50,乙组的血糖为4.16 10.77  9.08  5.95  6.36  3.77  5.18  6.76  3.86  3.63。假定甲乙两组血糖均服从正态分布，甲组的方差为5.0，乙组的方差为5.2，试求这两组平均血糖差的置信区间(取$\alpha=0.05$)
```{r}
...
例 假设例题8中两组的方差未知，试求这两组平均血糖差的置信区间(取$\alpha=0.05$)
```{r}
...
例 假设例题9中两组的方差未知，试求这两组平均血糖方差比的置信区间(取$\alpha=0.05$)
```{r}
var.test(x,y)
```
...
例 在某小学随机抽取了120人，发现其中34人有不同程度的视力下降，假定样本的数量服从正态分布，以95%的置信度, 估计这个小学视力下降比例。
...
例 对某疾病进行调查。在甲地区调查了160人，有98人符合诊断标准，在乙地区调查了206人，有132人符合诊断标准。试以95%的可靠性对该病在两地差别作出区间估计。
```{r}
s <- c(98,132)
t <- c(160,206)
prop.test(s,t)
...
例 对mtcars数据集中的mpg变量估算其中位数的置信区间，将wt和disp作为自变量，mpg 作为因变量，进行回归后对判定系数R-square，用自助法求它的95%置信区间。
```{r}
...
results=boot(data=mtcars,statistic=myfun ,R=1000)

#results这个数据结构中包括了原始样本的统计(results$t0)
#和再抽样样本的统计量(results$t0)
boot.ci(results,conf=0.95,type=c('perc','bca')) 
#其中conf表示置信水平，type表示了用何种算法来求区间，
#perc即使用百分位方法，bca表示adjusted bootstrap percentile，即对偏差进行了调整
...
print(results)
plot(results) 
#左侧的直方图表示了再抽样样本的统计量的经验分布，
#其中的虚线表示了原始样本的统计量，从中可以观察到偏差。
#右侧QQ图有助于判断经验分布是否正态。
boot.ci(results,conf=0.95,type=c('perc','bca'))

```

例 假定例题10的样本数量的分布状况未知，以95%的置信度, 估计这个小学视力下降比例。
...
OneSampleMean.Equality2 <- function(alpha, beta, sigma, margin,m=1000)
{
  ...
  while(abs(n1-n0)>0.5){
    n0<-((qt(alpha/2,n1,lower.tail=FALSE)+
            qt(beta,n1,lower.tail=FALSE))*sigma/margin)^2
    n1<-((qt(alpha/2,n0,lower.tail=FALSE)+
            qt(beta,n0,lower.tail=FALSE))*sigma/margin)^2
  }
  n1
}
...
```{r}
k <- c(2:20)
dim <- c("alpha0.01beta0.2","alpha0.05beta0.2","alpha0.01beta0.1",
         "alpha0.05beta0.1")
data <- c(11.68,13.89,15.46,16.75,17.87,18.88,19.79,20.64,21.43,
          22.18,22.89,23.57,
          24.22,24.84,25.44,26.02,26.58,27.12,27.65,7.85,9.64,
          10.91,11.94,12.83,13.63,
          14.36,15.03,15.65,16.25,16.81,17.34,17.85,18.34,18.82,
          19.27,19.71,20.14,
          20.56, 14.88,17.43,19.25,20.74,22.03,23.19,24.24,25.22,
          26.13,26.99,27.80,
          28.58,29.32,30.04,30.73,31.39,32.04,32.66,33.27,10.51,
          12.66,14.18,15.41,
          16.47,17.42,18.29,19.09,19.83,20.54,21.20,21.84,22.44,
          23.03,23.59,24.13,24.65,25.16,25.66 )
...
MutiSampleMean <- function(alpha, beta, sigma, k, mu){
  lambda <- z[k-1,paste(paste("alpha",alpha,sep = ""),
                        paste("beta",beta,sep = ""),
                        sep = "")]
...
```{r}
OneSampleProportion.Equivalence2 <- function(alpha, beta, p, 
                                             delta, margin) 
{
...
```{r}
TwoSampleProportion.NIS2 <- function (alpha, beta, p1, p2, 
                                      k, delta, margin) 
{
    n2 <- (qnorm(1 - alpha) + qnorm(1 - beta))^2 * 
      (p1 * (1 - p1)/k + p2 * (1 - p2))/(margin-delta)^2
    n1 <- k * n2
    n1
}
...
```{r}
TwoSampleProportion.Equivalence2 <- function (alpha, beta, p1, p2,
                                              k, delta, margin) 
{
    n2 <- (qnorm(1 - alpha) + qnorm(1 - beta/2))^2 * 
      (p1 * (1 - p1)/k + p2 * (1 - p2))/(delta- abs(margin))^2
    n1 <- k * n2
    n1
}
...
```{r}
TwoSamplePropCrossOver.Equality <- function (alpha, beta, 
                                             sigma, margin) 
{
...
```{r}
TwoSamplePropCrossOver.NIS <- function (alpha, beta, 
                                        sigma, delta,margin) 
{
    n <- (qnorm(1 - alpha) + qnorm(1 - beta))^2 * 
      sigma^2/2*(margin-delta)^2
    n
}
...
```{r}
TwoSamplePropCrossOver.Equivalence <- function (alpha, beta, 
                                                sigma, delta,margin) 
{
    n <- (qnorm(1 - alpha) + qnorm(1 - beta/2))^2 * 
      sigma^2/2*(delta-abs(margin))^2
    n
}
...
```{r}
PorpWilliamsDesign.Equality <- function (alpha, beta, 
                                         sigma, a, margin) 
{
    n <- (qnorm(1 - alpha/2) + qnorm(1 - beta))^2 * 
      sigma^2/(a *  margin^2)
...
```{r}
PropWilliamsDesign.NIS <- function (alpha, beta, 
                                    sigma, a, delta, margin) 
{
  n <- (qnorm(1 - alpha) + qnorm(1 - beta))^2 * 
    sigma^2/(a * (margin - delta)^2)
  n
}
...
```{r}
PropWilliamsDesign.Equivalence <- function (alpha, beta, 
                                            sigma, a, delta, margin) 
{
  n <- (qnorm(1 - alpha) + qnorm(1 - beta/2))^2 * 
    sigma^2/(a * (delta-abs(margin ))^2)
  n
}
...
```{r}
OR <- (0.2/(1-0.2))/(0.1/(1-0.1))
RelativeRiskCrossOver.NIS <- function (alpha, beta, 
                                       sigma, or,delta) 
{
    n <- (qnorm(1 - alpha) + qnorm(1 - beta))^2 * 
      sigma^2/(log(or) - delta)^2
    n
}
...
```{r}
OR <- (0.2/(1-0.2))/(0.1/(1-0.1))
RelativeRiskCrossOver.Equivalence <- function (alpha, beta, 
                                               sigma, or, delta) 
{
    n <- (qnorm(1 - alpha) + qnorm(1 - beta/2))^2 * 
      sigma^2/(delta- abs(log(or)))^2
    n
}
...
data<- c(3,7,11,16,21,26,30,35,38,41,45,43,41,39,38,27,27,40,48,
         56,62,67,68,71,70,69,
         70,62,55,49,45,30,4,9,14,21,27,35,41,45,52,54,58,58,
         55,54,46,39,38,55,64,77,
         83,93,96,94,98,93,92,85,75,69,55,44,2,5,8,12,15,19,
         21,24,28,31,31,32,33,29,
         25,22,21,31,37,44,46,50,49,50,53,53,49,47,45,37,30,
         25,3,6,11,16,20,26,30,35,
         39,41,44,43,44,41,33,28,32,40,53,61,64,71,72,75,
         75,72,71,64,61,53,40,32,2,5,
         7,11,13,16,19,22,24,23,25,26,24,23,20,13,16,25,
         28,35,36,39,41,42,42,37,37,
         36,31,28,23,14,3,6,9,14,17,21,24,28,30,32,33,
         32,32,30,25,13,25,33,38,47,49,
         53,53,56,54,53,50,45,42,37,29,14,2,3,5,7,9,
         12,13,15,16,17,19,17,16,17,13,
         10,16,18,22,24,26,30,29,30,29,28,29,24,21,21,
         15,11,2,4,7,10,13,15,19,20,24,
         23,25,25,24,20,17,10,20,25,32,36,39,39,44,
         41,44,39,39,36,32,25,20,11)
...
```{r}
dim1 <- c("p1","p2","alpha0.10beta0.20","alpha0.10beta0.10",
          "alpha0.05beta0.20","alpha0.05beta0.10")
p <- seq(1:14)
data1 <-c(0.05,0.10,0.15,0.20,0.25,0.30,0.35,0.40,0.45,
          0.50,0.55,0.60,0.65,0.70,
          0.30,0.35,0.40,0.45,0.50,0.55,0.60,0.65,0.70,
          0.75,0.80,0.85,0.90,0.95,
          25,31,34,39,40,41,41,41,41,40,39,34,31,25,
          33,41,48,52,56,57,57,57,57,
          56,52,48,41,33,34,39,46,49,54,55,56,56,55,
          54,49,46,39,34,42,52,60,65,
          71,72,77,77,72,71,65,60,52,42)
p0.25 <- array(data1,c(14,6),dimnames<-list(p,dim1))

p <- seq(1:12)
data2 <- c(0.05,0.10,0.15,0.20,0.25,0.30,0.35,0.40,0.45,
           0.50,0.55,0.60,0.35,0.40,
           0.45,0.50,0.55,0.60,0.65,0.70,0.75,0.80,0.85,
           0.90,20,23,26,28,29,29,
           33,29,29,28,26,23,26,32,35,39,40,40,40,40,40,
           39,35,32,25,30,34,36,
           37,41,41,41,37,36,34,30,33,39,45,47,51,53,
           53,53,51,47,45,39)
p0.30 <- array(data2,c(12,6),dimnames<-list(p,dim1))

p <- seq(1:12)
data3 <- c(0.05,0.10,0.15,0.20,0.25,0.30,0.35,0.40,0.45,
           0.50,0.55,0.60,0.40,0.45,
           0.50,0.55,0.60,0.65,0.70,0.75,0.80,0.85,0.90,
           0.95,16,19,20,23,24,24,24,
           24,23,20,19,16,21,24,28,29,29,33,33,29,29,28,
           24,21,20,24,26,27,30,31,
           31,30,27,26,24,20,25,31,34,36,36,40,40,36,
           36,34,31,25)
...
FisherTest <- function(alpha,beta,p1,p2){
  ...
  col <- paste(paste("alpha",alpha,sep=""),
               paste("beta",format(beta,nsmall=2),
                                                 sep=""),sep="")
  z[[table]][row,col]
}
...
gof.Pearson.twoway <- function (alpha, beta, trt, ctl, r, c) 
{
  ...
  noncen = 0
  for (i in 1:1000) {
    b[i + 1] <- pchisq(qchisq(alpha, df = (r-1)*(c-1),
                              ncp = 0, lower.tail = FALSE), 
                       df = (r-1)*(c-1), ncp = i/100)
...
```{r}
Logrank.test <- function(alpha, beta, k, year, loss, 
                         herate, hcrate, noncompliance, 
                         dropin, pe, pc) {
    ...
    T <- matrix(c(1, 0, 0, 0, 0, 1, 0, 0, 
                  transition(loss, k), transition(eeventrate, k), 
                  1 - transition(noncompliance, k) - 
                    transition(eeventrate, k) - transition(loss, k), 
                  transition(noncompliance, k), transition(loss, k), 
                  transition(ceventrate, k), transition(dropin, k), 
                  1 - transition(loss, k) - transition(ceventrate, k)
                  - transition(dropin, k)), nrow = 4)
    ...
    for (i in 1:(k * year)) {
        ...
        if (i == 1) {
            ...
            theta <- append(theta, log(1 - (E[[i]][2] - E[[i - 1]][2]))/
                              log(1 - (C[[i]][2] - C[[i - 1]][2])))  ##θ
            gamma <- append(gamma, phi[i] * theta[i]/
                              (1 + phi[i] * theta[i]) - 
                phi[i]/(1 + phi[i]))
            rho <- append(rho, ((E[[i]][2] - 
                                   E[[i - 1]][2]) + (C[[i]][2] - 
                C[[i - 1]][2]))/(pe + pc))  ##ρ
        }
    }
    ...
    d <- (qnorm(1 - alpha/2) + qnorm(1 - beta))^2 * 
...
panderOptions('digits', 4)
pandoc.table(data, use.hyphening = TRUE, 
             split.cells =8,justify = 'center')
```
...
Pocock.Test <- function(alpha, beta, sigma1, sigma2, mu, k) {
    nfixed <- (qnorm(1 - alpha/2) + qnorm(1 - beta))^2 * 
      (sigma1^2 + sigma2^2)/(mu)^2
    
    data <- read.csv("Rp.csv", header = T)
    col <- paste(paste("alpha", alpha, sep = ""), 
                 paste("beta", format(beta, 
        nsmall = 2), sep = ""), sep = "")
...
panderOptions('digits', 4)
pandoc.table(data, use.hyphening = TRUE, split.cells =8,
             justify = 'center')
```
...
WT.Test <- function(alpha, beta, sigma1, sigma2, mu, k, delta) {
    ...
    data <- read.csv("Rwt.csv", header = T)
    col <- paste(paste("delta", format(delta, nsmall = 2), sep = ""), 
                 paste("beta", 
...
InnerWedge.Test <- function(alphap,betap,sigma1,sigma2,mu,kp,deltap){
  nfixed <- (qnorm(1-alphap/2) + qnorm(1 - betap))^2*
    (sigma1^2+sigma2^2)/(mu)^2
  data <- read.csv("Rw.csv",header = T)
  data%>>%
    subset(beta==betap)%>>%
...
WT.Test.Binary <- function(alpha, beta, p1, p2, k, delta) {
    nfixed <- (qnorm(1 - alpha/2) + qnorm(1 - beta))^2 * 
      (p1 * (1 - p1) +  p2 * (1 - p2))/(p1 - p2)^2
    data <- read.csv("Rwt.csv", header = T)
    col <- paste(paste("delta", format(delta, nsmall = 2), 
                       sep = ""), paste("beta", 
        format(beta, nsmall = 2), sep = ""), sep = "")
    rwt <- as.matrix(data)[k, col]
    nmax <- rwt * nfixed
...
```{r}
options(digits = 4)
power.bd <- bounds(time, iuse = c(3,3), phi = c(2,2),
                   alpha = c(0.025, 0.025))
summary(power.bd)
plot(power.bd)
```
...
ISV.Equality <- function(alpha, beta, sigma1, sigma2, m) {
    ...
    n = 0
    for (i in 1:1000) {
        ratio.f = qf(p = (1 - beta), i * (m - 1), 
                     i * (m - 1), lower.tail = FALSE)/qf(alpha/2, 
...
ISV.NIS <- function(alpha, beta, sigma1, sigma2, m, margin) {
    ...
    for (i in 1:1000) {
        ratio.f = qf(p = (1 - beta), i * (m - 1),
                     i * (m - 1), lower.tail = FALSE)/qf(alpha, 
            i * (m - 1), i * (m - 1), lower.tail = FALSE)
        if (round(ratio, digits = 2) == round(ratio.f, digits = 2)) {
            n = i
        }
...
ISV.Equivalence <- function(alpha, beta, sigma1, sigma2, m, margin) {
    ...
    n = 0
    for (i in 1:1000) {
        ratio.f = qf(beta/2, i * (m - 1), 
                     i * (m - 1), lower.tail = FALSE)/qf(1 - 
...
ISCV.Equality <- function(alpha, beta, CVt, CVr, m) {
    ...
    n = (sigma1 + sigma2) * (qnorm(1 - alpha) +
                               qnorm(1 - beta))^2/(CVt - CVr)^2
    n
}
...
InterSV.Equality <- function(alpha, beta, vbt, vwt, vbr, vwr, m) {
    sigma = 2 * (vbt^2 + vwt^2/m)^2 + 
      (vbr^2 + vwr^2/m)^2 + vwt^4/(m^2 * 
        (m - 1)) + vwr^4/(m^2 * (m - 1))
    n = sigma * (qnorm(1 - alpha/2) + 
                   qnorm(1 - beta))^2/(vbt^2 - vbr^2)^2
    n
}
...
```{r}
InterSV.NIS <- function(alpha, beta, vbt, vwt, 
                        vbr, vwr, m, margin) {
    sigma = 2 * ((vbt^2 + vwt^2/m)^2 + 
                   margin^4 * (vbr^2 + vwr^2/m)^2 + 
        vwt^4/(m^2 * (m - 1)) +
          margin^4 * vwr^4/(m^2 * (m - 1)))
    n = sigma * (qnorm(1 - alpha) +
                   qnorm(1 - beta))^2/(vbt^2 - margin^2 * vbr^2)^2
    n
}
...
Variabilities.Parallel.Equality <- function(alpha, beta, vtt, vtr) {
    ...
    n = 0
    for (i in 2:1000) {
        ratio.f = qf(p = (1 - beta), i - 1, i - 1, 
                     lower.tail = FALSE)/qf(alpha/2, 
...
```{r}
Variabilities.Parallel.Equivalence <- function(alpha, beta, vtt, 
                                               vtr, margin) {
    ratio = margin^2 * vtt^2/vtr^2
    n = 0
    for (i in 2:1000) {
        ratio.f = qf(beta/2, i - 1, i - 1, 
                     lower.tail = FALSE)/qf(p = (1 - 
...
Variabilities.Parallel.Rep.Equality <- function(alpha, beta, vbt, vwt, 
    ...
    n = sigma * (qnorm(1 - alpha/2) + qnorm(1 - beta))^2/
      (vbt^2 + vwt^2 - (vbr^2 + vwr^2))^2
    n
}
...
Variabilities.Cross.Equality <- function(alpha, beta, vbt, vwt, vbr, vwr, 
    rho) {
    sigma = 2 * ((vbt^2 + vwt^2)^2 + (vbr^2 + vwr^2)^2 - 
                   2 * rho^2 * vbt^2 * vbr^2)
    ns = sigma * (qnorm(1 - alpha) + qnorm(1 - beta))^2/
      (vbt^2 + vwt^2 - (vbr^2 + vwr^2))^2
    ns
}
...
```{r}
Variabilities.Cross.NIS <- function(alpha, beta, vbt, 
                                    vwt, vbr, vwr, margin, rho) {
    sigma = 2 * ((vbt^2 + vwt^2)^2 + margin^4 * 
                   (vbr^2 + vwr^2)^2 - 2 * 
...
Variabilities.Cross.Rep.Equality <- function(alpha, beta, vbt, vwt, vbr, 
    vwr, m, rho) {
    sigma = 2 * ((vbt^2 + vwt^2/m)^2 + (vbr^2 + vwr^2/m)^2 - 
                   2 * rho^2 * vbt^2 * vbr^2 + (m - 1) * 
                   vwt^4/m^2 + (m - 1) * vwr^4/m^2)
    ns = sigma * (qnorm(1 - alpha/2) + 
                    qnorm(1 - beta))^2/(vbt^2 + vwt^2 - 
        (vbr^2 + vwr^2))^2
    ns
}
...
```{r}
Variabilities.Cross.Rep.NIS <- function(alpha, beta, vbt, 
                                        vwt, vbr, vwr, m, margin, rho) {
    sigma = 2 * ((vbt^2 + vwt^2/m)^2 + margin^4 * 
                   (vbr^2 + vwr^2/m)^2 - 
        2 * margin^2 * rho^2 * vbt^2 * 
          vbr^2 + (m - 1) * vwt^4/(m^2) + 
        margin^4 * (m - 1) * vwr^4/(m^2))
    ns = sigma * (qnorm(1 - alpha) + 
                    qnorm(1 - beta))^2/(vbt^2 + vwt^2 - 
        margin^2 * (vbr^2 + vwr^2))^2
    ns
}
...
IBE <- function(alpha, beta, delta, sigmaBT, sigmaBR, sigmaWT, sigmaWR, 
    ...
    U <- function(n, alpha, beta, delta, sigmaD, sigmaWT, sigmaWR, a, b, 
        thetaIBE) {
        U = ((abs(delta) + qt(alpha, 2 * n - 2) * 
                Sigma(sigmaD, sigmaWT, 
            sigmaWR, 0.5, 0.5)/2 * sqrt(2/n))^2 - 
              delta^2)^2 + Sigma(sigmaD, 
            sigmaWT, sigmaWR, 0.5, 0.5)^4 * 
          ((2 * n - 2)/qchisq(1 - alpha, 
            2 * n - 2) - 1)^2 + 0.5^2 *
          sigmaWT^4 * ((2 * n - 2)/qchisq(1 - 
            alpha, 2 * n - 2) - 1)^2 + 
          (1.5 + thetaIBE)^2 * sigmaWR^4 * 
            ((2 * n - 2)/qchisq(alpha, 2 * n - 2) - 1)^2
    }
    gamma = delta^2 + sigmaD^2 + sigmaWT^2 - 
      sigmaWR^2 - thetaIBE * sigmaWR^2
    n <- 0
    for (i in 2:1000) {
        bound = gamma + sqrt(U(i, alpha, 0.05, 
                               delta, sigmaD, sigmaWT, 
            sigmaWR, a, b, thetaIBE)) + 
          sqrt(U(i, alpha, beta, delta, sigmaD, 
...
```{r}
Vitro.BE <- function(alpha, beta, delta, sigmaBT, sigmaBR, 
                     sigmaWT, sigmaWR,  thetaBE) {
    U <- function(m, n, alpha, beta, delta, sigmaBT,
                  sigmaBR, sigmaWT, sigmaWR, thetaBE) {
        U = ((abs(delta) + qnorm(alpha) * 
                sqrt(sigmaBT^2/m + sigmaBR^2/m))^2 - 
            delta^2)^2+(sigmaBT^2 + sigmaWT^2)^2 *
          ((m - 1)/qchisq(1 - alpha, m - 1) - 1)^2
        +(1 + thetaBE)^2 *
          (sigmaBR^2 + sigmaWR^2)^2 * ((m - 1)/
                          qchisq(alpha,  m - 1) - 1)^2
    }
    ...
    n <- 0
    for (i in 2:1000) {
        bound = gamma + sqrt(U(i, 1, alpha, 0.05, 
                               delta, sigmaBT, sigmaBR, 
            sigmaWT, sigmaWR, thetaBE))
        +sqrt(U(i, 1, alpha, beta, delta, sigmaBT, 
                sigmaBR, sigmaWT, sigmaWR, 
            thetaBE))
        if (round(bound, digits = 2) == 0) {
            n = i
        }
...
ci=c(-6,1,2,3)
(Dose.Response.Linear(alpha=0.05,beta=0.2,
                      sigma=0.22,mui=mui,ci=ci,fi=1/4))
```
...
Ti=c(14,20,22,24);
ci=c(-6,1,2,3);
(Dose.Response.time.to.event(alpha=0.05,beta=0.2,
                             T0=9,T=16,Ti=Ti,ci=ci,fi=1/4))
```

每组需要`r round((Dose.Response.time.to.event(alpha=0.05,beta=0.2,T0=9,
T=16,Ti=Ti,ci=ci,fi=1/4)))/4`例。
...
```{r}
delta=c(rep(1,40/2),rep(1/2,40/2))
TwoSide.varyEffect(s1=100,s2=200,m=4000,m1=40,
                   delta=delta,a1=0.5,r1=24,fdr=0.01)
```
...
```{r}
who <- read.csv("WHO.csv", header = T)
plot(who$Deaths.due.to.HIV.AIDS..per.100.000.population.per.year., 
     who$Medical_Doctors)
...
col1 <- which(names(who) == 
                "Deaths.due.to.HIV.AIDS..per.100.000.population.per.year.")-1
...
```{r}
weight <- c(3.38,3.51,4.08,4.44,3.44,3.25,3.49,3.29,3.93,
            4.06,3.29,2.99,3.87,3.19,3.30,3.45,3.50,3.52,
            3.01,3.80,3.20,3.64,3.85,4.09,3.53,3.93,3.29,
            3.70,4.13,3.81,2.96,4.44,3.98,2.82,4.23)
...
```{r eval=FALSE}
tIndDf <- data.frame(DV=c(x, y),
                     IV=factor(rep(c("f", "m"), c(length(x),length(y)))))
...
例 某医生分别采用盐析法和结合法测定正常皮肤中胶原蛋白的含量。盐析法只能部分提纯，结合法较为复杂单精确。该医生欲寻求找盐析法和结合法之间的关系，以便通过盐析法预测结合法的测定值。
...
做散点图，观察是否存在线性关系。由于线性回归的条件（线性、正态性、方差齐性和独立性）是通过残差来完成的，可先建立回归方程，然后通过回归诊断来完成线性回归条件的检验。
线性条件可通过散点图直接观察。
```{r}
y <- c(546,553,562,563,570,575,581,605,607,621,
       624,626,632,640,656)
x <- c(6.8,7.8,8.7,8.7,8.9,9.5,10.1,10.2,10.3,
       10.4,11.1,12.4,13.3,13.1,13.2)
...
qqPlot(model,labels=row.names(df),
       id.method = "identify",simulate=T,main="QQPlot") #car包
```
...
ex <- rename(records, c("V1"="age","V2"="sbp",
            "V3"="dbp","V4"="wbc","V5"="mtfc"))
attach(ex)
#scatterplotMatrix()函数默认在非对角线区域绘制变量间的散点图，
#并添加平滑（loess）和线性拟合区间
...
subsets(sbs,legend=FALSE,statistic="cp",
...
MM2005 = function(formu,taus, data, group, pic=F){
  ...
  for( i in 1:length(taus) ){
    ya = cbind(1,engel1[,names(engel1)!=formu[[2]]] ) %*% 
      fita[[i]]$coef[,1]
    yb = cbind(1,engel2[,names(engel2)!=formu[[2]]] ) %*% 
      fitb[[i]]$coef[,1]
    ...
    tab[i,1] = fita[[i]]$tau
    tab[i,2] = yb - ya
    # 回报影响，数据相同，模型不同：模型机制的不同所产生的差异
    tab[i,3] = yb - ystar 
    # 变量影响，数据不同，模型相同：样本点不同产生的差异
    tab[i,4] = ystar - ya 
  }
  # 画图
  if( pic ){
    attach(engel)
    windows(5,5)
    plot(income, foodexp, cex=0.5, type="n",
         main="两组分位数回归结果比较")
...
goodfit <- function(glmFit)
{
  ...
  Nagelkerke <- as.vector((1 - exp((2/N) * (LL0 - LLf))) / 
                            (1 - exp(LL0)^(2/N)))
...
overdispersion <- function(fit.glm){
  ...
  fit.od <- glm(fit.glm$formula,family = quasibinomial, 
                data = fit.glm$data)
...
best.logistic <-bestglm(Affairs,family = binomial,
...
lrm(ynaffair ~ gender + age + yearsmarried + 
...
ciCoef <- t(apply(coefMN, 1, function(x) x["Estimate"] - 
                    zCrit*x["Std. Error"] ))
coefMN
ciCoef
```
...
vglm(intense.ord~agegr+shoes, family=acat(parallel=TRUE),
     data=HW93)
vglm(intense.ord~agegr+shoes, family=sratio(parallel=TRUE), 
     data=HW93)
```
...
exp(coefOrd[,1])
zCrit   <- qnorm(c(0.05/2, 1 - 0.05/2))
ciCoef <- t(apply(coefOrd, 1, function(x) x["Estimate"] - 
                    zCrit*x["Std. Error"] ))
```
...
drug.elrm=elrm(formula=recovered/n~sex+treatment,
               interest=~sex+treatment,iter=100000,
               burnIn=1000,dataset=drugDat)
summary(drug.elrm)
```
...
VGAM包vglm()方法获得类似结果
```{r,eval=FALSE}
vglm <- vglm(sumY~Base+Age+Trt, family=quasipoissonff, 
             data=breslow.dat)
summary(vglm)
```
...
```{r}
fit.rob <- glmRob(sumY~Base+Age+Trt, family = poisson(), 
                  data=breslow.dat)
summary(fit.rob)
```
...
library(mgcv)
```
加性模型是一种非参数模型，模型中每一个加性项使用单个光滑函数来估计，每一个加性项中可以解释因变量如何随的自变量变化而变化。广义加性模型时广义线性模型的扩展，
$$g(\mu )=s_{0}+s_{1}(x_{1})+s_{2}(x_{2})+\cdots +s_{p}(x_{p})$$
$$\eta =s_{0}+\sum_{i=1}^{p}s_{i}(X_{i})$$
其中$\eta$为线性预测值，$s_{i}$是非参数光滑函数，也可是光滑样条函数、核函数或者局部回归光滑函数，通过backfitting算法获得。模型不需要因变量对自变量的任何假设，由随机部分、加性部分及连接两者的连接函数$g(.)$组成。因变量的分布属于指数分布族、二项分布、Poisson分布、Gamma分布等。半参数广义加性模型，其形式为：
$$g(\mu )=s_{0}+X\beta +\sum_{i=1}^{p}s_{i}(X_{i})$$。
加性模型的拟合是通过一个迭代过程（向后拟合算法）对每个预测变量进行样条平滑。其算法要在拟合误差和自由度之间进行权衡最终达到最优。

例 robust包中Breslow癫痫数据记录了治疗初期八周内，抗癫痫药物对癫痫发病数的影响，因变量sumY为随机后8周内癫痫发病数，年龄Age和治疗前8周的癫痫发病数Base。

```{r}
data(breslow.dat,package="robust")
model=gam(sumY~s(Base)+s(Age),data=breslow.dat,
          family = poisson())
par(mfrow=c(1,2))
plot(model,se=T,resid=T,pch=16)
par(mfrow=c(1,1))
```
上图显示的是各预测变量的偏残差图，表示了各预测变量对响应变量的独立影响，纵轴括号中的数字表示EDF（estimated degrees of freedom），如果估计自由度为1，即是线性关系。
```{r}
summary(model)
```
从报告可以观察到各预测变量的EDF值，后面的P值表示平滑函数是否显著的减少了模型误差。伪判定系数R-sq显示了模型的解释能力为92.3%。

```{r}
model=gam(sumY~s(Base,k = 9)+s(Age,k=12),data=breslow.dat,family = poisson())
summary(model)
```
K的最小值是3，最大值是17，为3、4的时候都是直线，K越大，曲线原来越平滑，再大时，曲线就出现了一些弯曲，说明更精准了。

## 交互作用
考虑到Base和Age之间，可能有交互作用，采用s(Base,Age)方法
```{r}
data(breslow.dat,package="robust")
model=gam(sumY~s(Base,Age),data=breslow.dat,family = poisson())
summary(model)
```
```{r echo=F}
...
```{r}
a <- c(45, 44, 43, 47, 48, 44, 46, 44, 40, 45, 42, 40, 
       43, 46, 47, 45, 46, 45, 43, 44)
b <- c(45, 48, 47, 43, 46, 47, 48, 46, 43, 49, 46, 43, 
       47, 46, 47, 46, 45, 46, 44, 45, 46, 44, 43, 42, 45)
c <- c(47, 48, 45, 46, 46, 44, 45, 48, 49, 50, 49, 48,
       47, 44, 45, 46, 45, 43, 44, 45, 46, 43, 42)
...
#Fligner-Killeen(fligner.test()函数)和
#Brown-Forsythe检验(HH包中的hov()函数)也可以用来检验方差齐性
```
...
#   DescTools包中EtaSq(aovCRp, type=1)函数可以计算
(etaSq <- SSb / (SSb + SSw))
...
#pairwise.t.test(dfCRp$length, dfCRp$site, p.adjust.method="bonferroni")
...
plot(cld(tukey,level = .05),col="lightgrey")
```
...
调整。
```{r}
fitFiii <- lm(weight~gesttime+dose,
              contrasts=list(dose=contr.sum), data=litter)
...
fitIII <- lm(len ~ supp + dose + supp:dose, 
             data=ToothGrowth,
             contrasts=list(supp=contr.sum, dose=contr.sum))
...
interaction.plot(ToothGrowth$dose, ToothGrowth$supp,
                 ToothGrowth$len,main="Cell means", 
                 col=c("red", "blue", "green"), lwd=2) 
#interaction.plot(f1, f2, y)展示双因素方差分析的交互效应，
...
```{r eval=F}
#interaction2wt() 函数,因为它能展示任意
#复杂度设计(双因素方差分析、三因素方差分析等)
#的主效应(箱线图)和交互效应
...
aovCRFpq <- aov(len~ supp*dose, data=ToothGrowth)
summary(glht(aovCRFpq), 
        linfct=mcp(dose=cMat), alternative="two.sided",
...
###非计划Scheffe检验
```{r}
ScheffeTest(aovCRFpqA, which="comb", contrasts=t(cntrMat)) 
#Post-hoc Scheffe tests using the associated one-way ANOVA
ScheffeTest(aovCRFpq, which="dose", contrasts=c(-1, 1/2, 1/2)) 
...
#id和time需为因子
aovRBp <- aov(value ~ time + Error(id/time), data=dfRBpL) 
summary(aovRBp)
```
...
dfRBpW <- reshape(dfRBpL, v.names="value", timevar="time", idvar="id",
                  direction="wide")
fitRBp   <- lm(cbind(value.time0, value.time24,
                     value.time72) ~ 1, data=dfRBpW)
...
#如果不满足球形假设，可用Greenhouse-Geisser和 
#Huynh-Feldt校正或者用多变量方差分析
anova(fitRBp, M=~time, X=~1, idata=inRBp, test="Spherical") 
```
P大于0.05，符合球形假设。
...
```{r}
DVw<- data.matrix(subset(dfRBpW,select=c("value.time0", 
                                         "value.time24", "value.time72")))
diffMat <- combn(1:length(levels(dfRBpL$time)), 2, 
                 function(x) {DVw[ , x[1]] - DVw[ , x[2]]})
...
#dfRBFpqL$id <- as.factor(dfRBFpqL$id)
#这种法方法每组因子水平不是21，而是42
id <- factor(rep(1:21, times=2*3))  
dfRBFpqL$id <- id
aovRBFpq <- aov(value ~ treatment*time + Error(id/(treatment*time)), data=dfRBFpqL)
summary(aovRBFpq)

with(dfRBFpqL,interaction.plot(time,treatment, value, type = "b",
     col = c("red", "blue"), pch = c(16, 18), 
     main = "Interaction Plot for treatment and time"))
...
fitRBFpq   <- lm(cbind(value.contrast.time0,value.ps.time0,
                       value.contrast.time24,value.ps.time24,
                       value.contrast.time72,value.ps.time72) ~ 1,
                 data=dfRBFpqW)
...
anova(fitRBFpq, M=~treatment + time, 
      X=~treatment, idata=inRBFpq, test="Spherical")
anova(fitRBFpq, M=~treatment + time + treatment:time, 
      X=~treatment + time,idata=inRBFpq, test="Spherical")
mauchly.test(fitRBFpq, M=~treatment, X=~1, idata=inRBFpq)
mauchly.test(fitRBFpq, M=~treatment + time,
             X=~treatment, idata=inRBFpq)
mauchly.test(fitRBFpq, M=~treatment + time + treatment:time,
             X=~treatment + time, idata=inRBFpq)
```
...
###简单效应(Simple effects)
```{r}
summary(aov(value ~ treatment + Error(id/treatment),
            data=dfRBFpqL, subset=(time=="time0")))
summary(aov(value ~ treatment + Error(id/treatment), 
            data=dfRBFpqL, subset=(time=="time24")))
summary(aov(value ~ treatment + Error(id/treatment), 
            data=dfRBFpqL, subset=(time=="time72")))
```
...
diameter<- c(15.75,15.50,15.50,17.00,16.50,19.00,20.75,18.50,
             20.50,20.00,18.25,18.50,19.75,21.25,20.75,22.25
             ,21.50,23.50,24.75,23.75)
...
anova(fitSPFp.qr, M=~B + C + B:C, X=~B + C,
...
```{r}
anova(lme(Y ~ Xw1*Xw2, random=list(id=pdBlocked(list(~1, 
         pdIdent(~Xw1-1), pdIdent(~Xw2-1)))),
          method="ML", data=d2))
#符合compound symmetry假设
anova(lme(Y ~ Xw1*Xw2,
...
anova(lme(Y ~ Xb1*Xw1, random=list(id=pdCompSymm(~Xw1-1)),
          method="REML", data=d1))
```
...
anova(lme(Y ~ Xb1*Xw1*Xw2,
          random=list(id=pdBlocked(list(~1, pdIdent(~Xw1-1), 
                                        pdIdent(~Xw2-1)))),
          method="ML", data=d2))
#符合compound symmetry假设
anova(lme(Y ~ Xb1*Xw1*Xw2,
...
#####lme4包lmer()方法（Using lmer() from package lme4）
```{r}
anova(lmer(Y ~ Xb1*Xb2*Xw1*Xw2 + (1|id) + (1|Xw1:id) + 
             (1|Xw2:id), data=d2))
```
...
###生存曲线估计
```{r}
dfNew <- data.frame(Clinic=factor(c("1", "2"), 
                    levels=levels(addicts$Clinic)),
                    Dose=c(50, 60),Prison=factor(c("0", "1"), 
                     levels=levels(addicts$Prison)))
percs <- (1:99)/100
...
matplot(cbind(FWeib$fit[1, ],
        ...
        type="l", main=expression(paste("Weibull-Fit ", 
        hat(S)(t), " mit SE")),
        xlab="t", ylab="Survival", 
        lty=c(1, 2, 2), lwd=2, col="blue")
...
legend(x="topright", lwd=2, lty=c(1, 2, 1, 2), 
       ...
       legend=c("Clinic=1, Dose=50, Prison=0", 
      "+- 2*SE", "Clinic=2, Dose=60, Prison=1", "+- 2*SE"))
```
...
```{r}
dfNew  <- data.frame(Clinic=factor(c("2", "2"), 
       levels=levels(addicts$Clinic)),X=c(-2, -2),
...
plot(CPH, main=expression(paste("Cox PH-estimate ",
                                hat(S)(t), " with CI")),
...
legend=c("pseudo-observation", "Clinic=2, X=-2, Prison=0",
         "Clinic=2, X=-2, Prison=1"))
```
...
plot(hazard ~ time, main=expression(paste("Cox PH-estimate ", 
    hat(Lambda)[g](t), " per group")),
     type="s", ylim=c(0, 5), xlab="t",
    ylab="cumulative hazard", lwd=2, data=Lambda0A)
...
```{r}
plot(survfit(surv~Clinic,data=addicts),fun="cloglog",
     conf.int = F,col = c("red","blue"))
```
...
```{r}
dfbetas <- residuals(coxph(surv~Clinic+Prison,data=addicts),
                     type="dfbetas")
par(mfrow=c(1, 2))
plot(dfbetas[ , 1], type="h", main="DfBETAS for Clinic", 
     ylab="DfBETAS", lwd=2)
plot(dfbetas[ , 2], type="h", main="DfBETAS for Prison", 
     ylab="DfBETAS", lwd=2)
```
...
```{r}
resMart <- residuals(coxph(surv~Clinic+Dose,data=addicts), 
                     type="martingale")
par(mfrow=c(1, 1))
...
y <- c(4.17,4.95,3.96,3.59,4.89,3.03,3.71,5.91,5.55,6.29,
       4.82,3.90,6.11)
...
```{r}
x <- c(620.16,866.50,641.22,812.91,738.96,899.38,
       760.78,694.95,749.92,793.94)
y <- c(958.47,838.42,788.90,815.20,783.17,910.92,
       758.49,870.80,862.26,805.48)
...
```{r}
x <- c(4.98,3.90,4.02,0.68,4.98,5.04,1.20,2.64,6.23,3.00)
y <- c(4.17,4.95,3.96,3.59,4.89,3.03,3.71,5.91,5.55,
       6.29,4.82,3.90,6.11)
...
```{r}
Value <- c(1,2,5,3,2,1,1,3,2,1,4,3,6,5,2,6,1,6,5,4,9,6,
           7,7,5,1,8,9,6,5)
...
## true negative 真阴性
TN <- c11 <- contT1[1, 1]       
## true positive / hit 真阳性
TP <- c22 <- contT1[2, 2]       
## false positive 假阳性
FP <- c12 <- contT1[1, 2]       
## false negative / miss 假阴性
FN <- c21 <- contT1[2, 1]       
...
#正确分类率 accuracy = (TP + TN)/(TP + FP + FN + TN)
...
```{r}
# OR值
(OR <- OddsRatio(contT1, conf.level=0.95))
...
```{r}
set.seed(123)
myDf <- data.frame(work =factor(sample(c("home", "office"), 
                                       10, replace=TRUE)),
...
kappa.test <- function(x)
{
  ...
  k=(Po-Pe)/(1-Pe) 
  # kappa统计量的标准误
  se_k = sqrt((Pe+Pe^2-sum(mr*mc*(mr+mc)))/(N*(1-Pe)^2)) 
  #检验统计量
  z=k/se_k 
  # p值
  p.value=1-pnorm(z) 
...
addmargins(cTab)
#对于大于2*2的列联表mcnemar.test自动采用Bowke检验
mcnemar.test(cTab) 
#coin包
symmetry_test(cTab, teststat="quad", distribution=approximate(B=9999)) 
```
...
```{r}
a <- c(45, 44, 43, 47, 48, 44, 46, 44, 40, 45, 42, 40, 43, 
       46, 47, 45, 46, 45, 43, 44)
b <- c(45, 48, 47, 43, 46, 47, 48, 46, 43, 49, 46, 43, 47, 
       46, 47, 46, 45, 46, 44, 45, 46, 44, 43, 42, 45)

dfCRp <- data.frame(length = c(a, b), site = factor(c(rep("1", 20), 
    rep("2", 25))))

set.seed(123)
ot <- oneway_test(length ~ site, data = dfCRp,
...
plot(supp, dens, xlab="Support", ylab=NA, pch=20, 
     main="Density permutation distribution")
```
...
set.seed(123)
oneway_test(DV ~ IV | id, alternative="less", 
            distribution=approximate(B=9999), data=tDepDf)
```
...
```{r}
set.seed(123)
cmh_test(Treatment~Improved|Sex,
         distribution=approximate(B=9999),data=Arthritis)
```
P值较小，分性别来看，治疗情况和改善情况并不独立。
...
例 某医生为了探讨缺碘地区母婴TSH水平的关系，应用免疫放射分析测定了160名孕妇（15-17周）及分娩时脐带血TSH水平（mU/L），现随机抽取10对数据，母血TSH1.21 1.30 1.39	1.42 1.47 1.56 1.68	1.72 1.98	2.1,脐血TSH3.90 4.5 4.20	4.83 4.16 4.93 4.32	4.99 4.7 5.2，试对母血TSH水平与新生儿脐带血TSH水平进行相关分析。
```{r}
x <- c(1.21,3.90,1.30,4.50,1.39,4.20,1.42
       ,4.83,1.47,4.16)
y <- c(1.56,4.93,1.68,4.32,1.72,4.99,1.98
       ,4.70,2.10,5.20)
df <- as.data.frame(cbind(x,y))
...
Fdf <- data.frame(id=factor(rep(1:N, times=P)),
                  ...
                  IV=factor(rep(1:P, each=N),
                            labels=LETTERS[1:P]))
set.seed(123)
friedman_test(DV ~ IV | id, 
              distribution=approximate(B=9999), data=Fdf)
```
P值小于0.05，拒绝原假设，可以认为四种不同的测试方式之间有差异。
...
getAnovaWild <- function(dat, idx) {
  ...
  Uf <- sample(c(-(sqrt(5) - 1)/2, (sqrt(5) + 1)/2), 
               ...
               prob=c((sqrt(5) + 1)/(2*sqrt(5)), 
                      (sqrt(5) - 1)/(2*sqrt(5))))
  ## for E* with Rademacher-variables
  Ystar <- Yhat + (Er*Ur)[idx]          
  # Ystar <- Yhat + (Er*Uf)[idx]       
  ## for E* with 2nd option
  anBS  <- anova(lm(Ystar ~ site, data=dat))
  anBS["site", "F value"]
}
...
Affairs$ynaffair <- factor(Affairs$ynaffair,
                           levels=c(0,1),labels=c("No","Yes"))
fit<- glm(Affairs$ynaffair~gender+age+yearsmarried+
...
```{r}
data(breslow.dat,package="robust")
fitm <- glm(breslow.dat$sumY~Base+Age+Trt,
...
result <- boot(data = Affairs,statistic = bs,R=1000,
         formula=ynaffair~gender+age+yearsmarried+
         children+religiousness+education+occupation+rating)
print(result)
#多个统计量，添加索引
...
参数法
```{r}
tt <- t.test(DV ~ IV, alternative="two.sided", 
             var.equal=TRUE, data=tDf)
tt$conf.int
```
...
try(detach(package:lmPerm))
try(detach(package:pgirmess))
try(detach(package:plyr))
try(detach(package:boot))
```
