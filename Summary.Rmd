...
map <- get_googlemap("gansu", zoom = 6, maptype = "satellite", markers = dat.pts, path = dat.pts, scale = 2)
...
```{r echo=F}
try(detach(package:maptools))
try(detach(package:ggplot2))
try(detach(package:mapproj))
try(detach(package:ggmap))
try(detach(package:dplyr))
try(detach(package:magrittr))
try(detach(package:rgdal))
try(detach(package:animation))
try(detach(package:XML))
try(detach(package:RCurl))
try(detach(package:plyr))
try(detach(package:leaflet))
try(detach(package:rgdal))
try(detach(package:htmltools))
try(detach(package:reshape2))
```
