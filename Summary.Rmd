...
```{r}
data(breslow.dat,package="robust")
model=gam(sumY~s(Base)+s(Age),data=breslow.dat,family = poisson())
par(mfrow=c(1,2))
...
fitIII <- lm(len ~ supp + dose + supp:dose, 
             data=ToothGrowth,contrasts=list(supp=contr.sum, dose=contr.sum))
...
#interaction.plot(f1, f2, y)展示双因素方差分析的交互效应，如果f1和f2是因子,作y的均值图,以f1的不同值作为x轴,而f2的不同值对应不同曲线;可以用选项fun指定y的其他的统计量(缺省计算均值,fun=mean)
```
...
```{r eval=F}
#interaction2wt() 函数,因为它能展示任意复杂度设计(双因素方差分析、三因素方差分析等)
#的主效应(箱线图)和交互效应
...
fitRBFpq   <- lm(cbind(value.contrast.time0,value.ps.time0,
                       ...
                       value.contrast.time72,value.ps.time72) ~ 1,data=dfRBFpqW)
