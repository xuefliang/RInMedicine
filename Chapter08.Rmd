...
```{r}
aovSPFpq <- aov(Diameter ~ B*A + Error(id/A), data=dfSPFpqL)
summary(aovSPFpq)
```
主因素和副因素均有统计学意义。
...
###宽数据格式
```{r}
dfSPFpqW <- reshape(dfSPFpqL, v.names="Diameter", timevar="A",
                    idvar=c("id", "B"), direction="wide")

fitSPFpq   <- lm(cbind(Diameter.1, Diameter.2) ~ B, data=dfSPFpqW)
inSPFpq    <- data.frame(A=gl(2, 1))
AnovaSPFpq <- Anova(fitSPFpq, idata=inSPFpq, idesign=~A)
...
anova(fitSPFpq, M=~A, X=~1, idata=inSPFpq, test="Spherical")
mauchly.test(fitSPFpq, M=~A, X=~1, idata=inSPFpq)
```
...
summary(aov(Diameter ~ B, data=dfSPFpqL, subset=(A==1)))
summary(aov(Diameter ~ B, data=dfSPFpqL, subset=(A==2)))
#Within-subjects effect at a fixed level of the between-subjects factor
summary(aov(Diameter ~ A + Error(id/A), data=dfSPFpqL,subset=(B==1)))
summary(aov(Diameter ~ A + Error(id/A), data=dfSPFpqL,subset=(B==2)))
```

###计划的多重比较（Planned comparisons for the between-subjects factor）
```{r}
mDf    <- aggregate(Diameter ~ id + B, data=dfSPFpqL, FUN=mean)
aovRes <- aov(Diameter ~ B, data=mDf)
cMat <- rbind("1-2"=c(-1, 1))
summary(glht(aovRes, linfct=mcp(B=cMat), alternative="greater"),
        test=adjusted("none"))
```
副因素之间的差异具有统计学意义。
...
例 d1为长格式的重复观测数据，因变量为Y，自变量为Xw1，id为表示变量。d2为长格式的重复观测数据，因变量为Y，自变量为Xw1和Xw2，id为表示变量。
```{r}
